...
func TestBackendNilKeyBlockSearchTraceQL(t *testing.T) {
	numTraces := 100
	traces := make([][]FlatSpan, 0, numTraces)
	...
	for i := 0; i < numTraces; i++ {
		...
		flatSpans, _ := traceToParquet(&backend.BlockMeta{}, id, test.MakeTrace(1, id), nil)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for {
				...
				for _, sp := range spanSet.Spans {
					spansToCheck = append(spansToCheck, sp.ID())
				}

				// check all the matching spans in the trace to make sure the attribute does not exist
				for _, trSpans := range traces {
					if len(trSpans) == 0 {
						continue
					}
					if bytes.Equal(trSpans[0].TraceID, spanSet.TraceID) {
						// count unique resource service names as a proxy for resource count
						seenRes := map[string]struct{}{}
						for _, fs := range trSpans {
							seenRes[fs.ResourceServiceName] = struct{}{}
						}
						numResourcesFound += len(seenRes)

						for _, fs := range trSpans {
							for _, spanID := range spansToCheck {
								if bytes.Equal(spanID, fs.SpanID) {
									switch tc.level {
									case "span":
									...
									case "event":
										require.Greater(t, len(fs.Events), 0, fmt.Sprintf("span without events returned: %s", fs.SpanID))
										...
										if eventsWithAttrFound == len(fs.Events) && len(fs.Events) > 0 {
											found = true
											break
										}
									case "link":
										require.Greater(t, len(fs.Links), 0, fmt.Sprintf("span without links returned: %s", fs.SpanID))
										...
										if linksWithAttrFound == len(fs.Links) && len(fs.Links) > 0 {
											found = true
											break
										}
									}
...
func TestBackendNilValueBlockSearchTraceQL(t *testing.T) {
	...
	for i := 0; i < numTraces; i++ {
		if i == wantTraceIdx {
			wantTrace := []FlatSpan{
				{
					// span 0: nil values for everything except SpanID and one generic attr
					TraceID:             wantTraceID,
					ResourceServiceName: "hello",
				...
				{
					// span 1: has all span values, resource level nils
					TraceID:             wantTraceID,
					ResourceServiceName: "hello",
					ResourceAttrs: []Attribute{
						attr("foo", "def"),
				...
				{
					// span 2: has resource values, span level nils
					TraceID:                        wantTraceID,
					ResourceServiceName:            "myservice",
					ResourceDroppedAttributesCount: 22,
					ResourceAttrs: []Attribute{
						attr("foo", "abc"),
						attr("str-array", []string{"value-one", "value-two", "value-three", "value-four"}),
						attr("int-array", []int64{11, 22, 33}),
						attr(LabelServiceName, 123),
					},
...
func fullyPopulatedTestTrace(id common.ID) []FlatSpan {
	return fullyPopulatedTestTraceWithOption(id, false)
}

func fullyPopulatedTestTraceWithOption(id common.ID, parentIDTest bool) []FlatSpan {
	...
	serviceStats := []ServiceStats{
		{ServiceName: "myservice", SpanCount: 1, ErrorCount: 0},
		{ServiceName: "service2", SpanCount: 1, ErrorCount: 0},
	}
	...
	return []FlatSpan{
		{
			...
			TraceDurationNano:      uint64((100 * time.Millisecond).Nanoseconds()),
		...
		},
		{
			...
			ResourceDedicatedString01: []string{"dedicated-resource-attr-value-6"},
			ResourceDedicatedString02: []string{"dedicated-resource-attr-value-7"},
			ResourceDedicatedString03: []string{"dedicated-resource-attr-value-8"},
			ResourceDedicatedString04: []string{"dedicated-resource-attr-value-9"},
			ResourceDedicatedString05: []string{"dedicated-resource-attr-value-10"},
			// Scope-level
			ScopeName:    "scope-2",
			ScopeVersion: "version-2",
			ScopeAttrs: []Attribute{
...
func mixedArrayTestTrace() ([]FlatSpan, backend.DedicatedColumns) {
	...
	return []FlatSpan{
		{
			TraceID:             traceID,
			ResourceServiceName: "flat",
			ResourceAttrs: []Attribute{
				attr("rs-generic", "a"),
				attr("ri-generic", 1),
			},
			ResourceDedicatedString01: []string{"a"},
			ResourceDedicatedInt01:    []int64{1},
			SpanID:                    []byte{},
			ParentSpanID:              []byte{},
			Name:                      "flat",
			Attrs: []Attribute{
			...
			DedicatedString01: []string{"b"},
			DedicatedInt01:    []int64{2},
			Events: []Event{
				{
					Name: "flat",
					Attrs: []Attribute{
		...
		{
			TraceID:             traceID,
			ResourceServiceName: "arrays",
			ResourceAttrs: []Attribute{
				attr("rs-generic", []string{"a", "b"}),
				attr("ri-generic", []int64{1, 2}),
			},
			ResourceDedicatedString01: []string{"b", "c"},
			ResourceDedicatedInt01:    []int64{1},
			SpanID:                    []byte{},
			ParentSpanID:              []byte{},
			Name:                      "arrays",
			...
			DedicatedString01: []string{"c", "d"},
			DedicatedInt01:    []int64{3, 4},
			Events: []Event{
				{
					Name: "event-arrays",
					Attrs: []Attribute{
...
func trimForSelectAll(spans []FlatSpan) {
	for i := range spans {
		spans[i].ResourceAttrs = trimArrayAttrs(spans[i].ResourceAttrs)
		spans[i].Attrs = trimArrayAttrs(spans[i].Attrs)
	}
}

func flattenForSelectAll(spans []FlatSpan, dcm dedicatedColumnMapping) *traceql.Spanset {
	if len(spans) == 0 {
	...
	newSS := &traceql.Spanset{
		RootServiceName: first.RootServiceName,
		RootSpanName:    first.RootSpanName,
		TraceID:         first.TraceID,
		DurationNanos:   first.TraceDurationNano,
	}
	traceAttrs = append(traceAttrs, attrVal{traceql.IntrinsicTraceIDAttribute, traceql.NewStaticString(first.TraceIDText)})
	traceAttrs = append(traceAttrs, attrVal{traceql.IntrinsicTraceDurationAttribute, traceql.NewStaticDuration(time.Duration(first.TraceDurationNano))})
	traceAttrs = append(traceAttrs, attrVal{traceql.IntrinsicTraceRootServiceAttribute, traceql.NewStaticString(first.RootServiceName)})
	traceAttrs = append(traceAttrs, attrVal{traceql.IntrinsicTraceRootSpanAttribute, traceql.NewStaticString(first.RootSpanName)})
	...
	for i := range spans {
		...
		// Resource attrs
		var rsAttrs []attrVal
		rsAttrs = append(rsAttrs, attrVal{traceql.NewScopedAttribute(traceql.AttributeScopeResource, false, LabelServiceName), traceql.NewStaticString(s.ResourceServiceName)})

		for _, a := range parquetToProtoAttrs(s.ResourceAttrs) {
		...
		for attr, col := range dcm.items() {
			if strings.Contains(col.ColumnPath, "Resource") {
				v := col.readResourceValue(s)
				if v == nil {
					a := traceql.NewScopedAttribute(traceql.AttributeScopeResource, false, attr)
					sv := traceql.NewStaticString("nil")
					rsAttrs = append(rsAttrs, attrVal{a, sv})
					continue
				}
				a := traceql.NewScopedAttribute(traceql.AttributeScopeResource, false, attr)
				sv := traceql.StaticFromAnyValue(v)
				rsAttrs = append(rsAttrs, attrVal{a, sv})
			}
		}
		sortAttrs(rsAttrs)
		...
		instrumentationAttrs = append(instrumentationAttrs, attrVal{traceql.IntrinsicInstrumentationNameAttribute, traceql.NewStaticString(s.ScopeName)})
		instrumentationAttrs = append(instrumentationAttrs, attrVal{traceql.IntrinsicInstrumentationVersionAttribute, traceql.NewStaticString(s.ScopeVersion)})
		for _, a := range parquetToProtoAttrs(s.ScopeAttrs) {
			if arr := a.Value.GetArrayValue(); arr != nil {
				for _, v := range arr.Values {
		...
		for attr, col := range dcm.items() {
			if strings.HasPrefix(col.ColumnPath, "Dedicated") {
				v := col.readSpanValue(s)
				if v == nil {
					a := traceql.NewScopedAttribute(traceql.AttributeScopeSpan, false, attr)
					sv := traceql.NewStaticString("nil")
					newS.addSpanAttr(a, sv)
					continue
				}
				a := traceql.NewScopedAttribute(traceql.AttributeScopeSpan, false, attr)
				sv := traceql.StaticFromAnyValue(v)
