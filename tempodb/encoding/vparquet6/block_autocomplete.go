...
func oneLevelUp(definitionLevel int) int {
	...
	case DefinitionLevelResourceSpansILSSpanAttrs: // Same as DefinitionLevelResourceAttrs in flat schema
		return DefinitionLevelResourceSpansILSSpan
	case DefinitionLevelResourceSpansILSSpanEventAttrs: // covers links as well
...
func scopeFromDefinitionLevel(_ int, keyPath string) traceql.AttributeScope {
	// In the flat schema resource/scope/span attrs all share definition level 1,
	// so we disambiguate by key column path instead.
	switch keyPath {
	case columnPathSpanAttrKey:
		return traceql.AttributeScopeSpan
	case columnPathResourceAttrKey:
		return traceql.AttributeScopeResource
	case columnPathInstrumentationAttrKey:
