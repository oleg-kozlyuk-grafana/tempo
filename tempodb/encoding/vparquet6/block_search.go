...
func makePipelineWithRowGroups(ctx context.Context, req *tempopb.SearchRequest, pf *parquet.File, rgs []parquet.RowGroup, dc backend.DedicatedColumns) pq.Iterator {
	...
	if req.MinDurationMs > 0 || req.MaxDurationMs > 0 {
		...
		traceIters = append(traceIters, makeIter(columnPathDurationNanos, durFilter, "Duration"))
	}
	...
	if req.Start > 0 && req.End > 0 {
		...
		traceIters = append(traceIters, makeIter(columnPathStartTimeUnixNano, startFilter, "StartTime"))

		// Trace end >= req.Start, only if column exists
		if pq.HasColumn(pf, columnPathEndTimeUnixNano) {
			endFilter := pq.NewIntBetweenPredicate(time.Unix(int64(req.Start), 0).UnixNano(), math.MaxInt64)
			traceIters = append(traceIters, makeIter(columnPathEndTimeUnixNano, endFilter, ""))
		}
	}
	...
	case 0:
		...
		return makeIter(columnPathTraceID, nil, "")
...
func rawToResults(ctx context.Context, pf *parquet.File, rgs []parquet.RowGroup, rowNumbers []pq.RowNumber) ([]*tempopb.TraceSearchMetadata, error) {
	...
	iter2 := pq.NewJoinIterator(DefinitionLevelTrace, []pq.Iterator{
		...
		makeIter(columnPathTraceID, nil, "TraceID"),
		makeIter(columnPathRootServiceName, nil, "RootServiceName"),
		makeIter(columnPathRootSpanName, nil, "RootSpanName"),
		makeIter(columnPathStartTimeUnixNano, nil, "StartTimeUnixNano"),
		makeIter(columnPathDurationNanos, nil, "DurationNano"),
	}, nil)
	defer iter2.Close()
