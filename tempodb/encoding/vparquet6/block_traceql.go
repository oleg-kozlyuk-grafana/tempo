...
const (
	...
	columnPathStartTimeUnixNano       = "TraceStartTimeUnixNano"
	columnPathEndTimeUnixNano         = "TraceEndTimeUnixNano"
	columnPathDurationNanos           = "TraceDurationNano"
	...
	columnPathResourceAttrKey         = "ResourceAttrs.Key"
	columnPathResourceAttrString      = "ResourceAttrs.Value"
	columnPathResourceAttrInt         = "ResourceAttrs.ValueInt"
	columnPathResourceAttrDouble      = "ResourceAttrs.ValueDouble"
	columnPathResourceAttrBool        = "ResourceAttrs.ValueBool"
	ColumnPathResourceServiceName     = "ResourceServiceName"

	columnPathInstrumentationName       = "ScopeName"
	columnPathInstrumentationVersion    = "ScopeVersion"
	columnPathInstrumentationAttrKey    = "ScopeAttrs.Key"
	columnPathInstrumentationAttrString = "ScopeAttrs.Value"
	columnPathInstrumentationAttrInt    = "ScopeAttrs.ValueInt"
	columnPathInstrumentationAttrDouble = "ScopeAttrs.ValueDouble"
	columnPathInstrumentationAttrBool   = "ScopeAttrs.ValueBool"

	columnPathSpanID               = "SpanID"
	ColumnPathSpanName             = "Name"
	columnPathSpanStartTime        = "StartTimeUnixNano"
	columnPathSpanStartRounded15   = "StartTimeRounded15"
	columnPathSpanStartRounded60   = "StartTimeRounded60"
	columnPathSpanStartRounded300  = "StartTimeRounded300"
	columnPathSpanStartRounded3600 = "StartTimeRounded3600"
	columnPathSpanDuration         = "DurationNano"
	columnPathSpanKind             = "Kind"
	columnPathSpanStatusCode       = "StatusCode"
	columnPathSpanStatusMessage    = "StatusMessage"
	columnPathSpanAttrKey          = "Attrs.Key"
	columnPathSpanAttrString       = "Attrs.Value"
	columnPathSpanAttrInt          = "Attrs.ValueInt"
	columnPathSpanAttrDouble       = "Attrs.ValueDouble"
	columnPathSpanAttrBool         = "Attrs.ValueBool"
	columnPathSpanNestedSetLeft    = "NestedSetLeft"
	columnPathSpanNestedSetRight   = "NestedSetRight"
	columnPathSpanChildCount       = "ChildCount"
	columnPathSpanParentID         = "ParentID"
	columnPathSpanParentSpanID     = "ParentSpanID"
	ColumnPathEventName            = "Events.Name"
	columnPathEventTimeSinceStart  = "Events.TimeSinceStartNano"
	columnPathLinkTraceID          = "Links.TraceID"
	columnPathLinkSpanID           = "Links.SpanID"
	columnPathEventAttrKey         = "Events.Attrs.Key"
	columnPathEventAttrString      = "Events.Attrs.Value"
	columnPathEventAttrInt         = "Events.Attrs.ValueInt"
	columnPathEventAttrDouble      = "Events.Attrs.ValueDouble"
	columnPathEventAttrBool        = "Events.Attrs.ValueBool"
	columnPathLinkAttrKey          = "Links.Attrs.Key"
	columnPathLinkAttrString       = "Links.Attrs.Value"
	columnPathLinkAttrInt          = "Links.Attrs.ValueInt"
	columnPathLinkAttrDouble       = "Links.Attrs.ValueDouble"
	columnPathLinkAttrBool         = "Links.Attrs.ValueBool"

	// Flat schema definition level aliases.
	// In the flat format one Parquet row = one span, so trace/resource/scope/span
	// are all at definition level 0. Events and Links are the only nested lists.
	DefinitionLevelTrace                          = DefinitionLevelSpan       // 0
	DefinitionLevelServiceStats                   = DefinitionLevelSpanAttrs  // 1 - ServiceStats is a repeated group
	DefinitionLevelResourceSpans                  = DefinitionLevelSpan       // 0
	DefinitionLevelResourceAttrs                  = DefinitionLevelSpanAttrs  // 1 - ResourceAttrs is a repeated group
	DefinitionLevelInstrumentationScope           = DefinitionLevelSpan       // 0
	DefinitionLevelInstrumentationScopeAttrs      = DefinitionLevelSpanAttrs  // 1 - ScopeAttrs is a repeated group
	DefinitionLevelResourceSpansILSSpan           = DefinitionLevelSpan       // 0
	DefinitionLevelResourceSpansILSSpanAttrs      = DefinitionLevelSpanAttrs  // 1
	DefinitionLevelResourceSpansILSSpanEvent      = DefinitionLevelEvent      // 1
	DefinitionLevelResourceSpansILSSpanLink       = DefinitionLevelLink       // 1
	DefinitionLevelResourceSpansILSSpanEventAttrs = DefinitionLevelEventAttrs // 2
	DefinitionLevelResourceSpansILSSpanLinkAttrs  = DefinitionLevelLinkAttrs  // 2
	...
	// spanIDDriverKey is used to distinguish the spanID column when it's used as
	// the row driver from when it's explicitly requested by a condition.
	// The driver's spanID should set sp.id but NOT count as a matched attribute.
	spanIDDriverKey = "__spanID_driver"
...
func fetch(ctx context.Context, req traceql.FetchSpansRequest, pf *parquet.File, rowGroups []parquet.RowGroup, dc backend.DedicatedColumns) (*spansetIterator, error) {
	...
	// In the flat schema each row is one span. The pipeline above produces one spanset
	// per row. Group consecutive same-TraceID spansets into a single spanset per trace.
	grouped := newTraceGroupIterator(newRebatchIterator(iter))
	return newSpansetIterator(grouped), nil
}
...
func createSpanIterator(makeIter, makeNilIter makeIterFn, innerIterators []parquetquery.Iterator, conditions []traceql.Condition, allConditions bool, dedicatedColumns backend.DedicatedColumns, selectAll bool,
	...
	// In the flat schema, each row is one span. Use SpanID column directly to iterate all rows.
	if len(required) == 0 {
		var pred parquetquery.Predicate
		if sampler != nil {
			pred = newSamplingPredicate(sampler, nil)
		}
		required = []parquetquery.Iterator{makeIter(columnPathSpanID, pred, spanIDDriverKey)}
	}
...
func createTraceIterator(makeIter makeIterFn, resourceIter parquetquery.Iterator, conds []traceql.Condition, start, end uint64, allConditions bool, selectAll bool,
	...
	traceIter, err := parquetquery.NewLeftJoinIterator(DefinitionLevelTrace, required, iters, newTraceCollector(), parquetquery.WithPool(pqTracePool))
	if err != nil {
		return nil, err
	}
...
func (c *spanCollector) KeepGroup(res *parquetquery.IteratorResult) bool {
	...
	for _, kv := range res.Entries {
		switch kv.Key {
		case spanIDDriverKey:
			// Driver only - don't set sp.id to avoid inflating attributesMatched() count.
			// sp.id is set later when SpanID is explicitly requested via condition or selectAll.
...
// traceGroupIterator reads all results from the inner iterator, groups spansets
// by TraceID, and yields one merged spanset per trace. In the flat schema each
// parquet row is one span, so the inner pipeline produces one spanset per span.
// This iterator merges them into one spanset per trace at the end of the pipeline.
type traceGroupIterator struct {
	inner  parquetquery.Iterator
	groups []*traceql.Spanset // pre-built grouped results
	idx    int
	at     parquetquery.IteratorResult
}

var _ parquetquery.Iterator = (*traceGroupIterator)(nil)

func newTraceGroupIterator(inner parquetquery.Iterator) *traceGroupIterator {
	return &traceGroupIterator{inner: inner}
}

func (g *traceGroupIterator) String() string {
	return fmt.Sprintf("traceGroupIterator(%s)", g.inner)
}

func (g *traceGroupIterator) init() error {
	// Read all results and group by TraceID
	traceIndex := map[string]int{} // traceID hex → index in groups

	for {
		res, err := g.inner.Next()
		if err != nil {
			return err
		}
		if res == nil {
		...
		key := string(ss.TraceID)
		if idx, exists := traceIndex[key]; exists {
			// Merge into existing group
			g.groups[idx].Spans = append(g.groups[idx].Spans, ss.Spans...)
			for k, v := range ss.ServiceStats {
				g.groups[idx].ServiceStats[k] = v
			}
			putSpanset(ss)
		} else {
			// New trace group
			traceIndex[key] = len(g.groups)
			g.groups = append(g.groups, ss)
		}
	}
	return nil
}

func (g *traceGroupIterator) Next() (*parquetquery.IteratorResult, error) {
	// Lazily initialize on first call
	if g.groups == nil && g.idx == 0 {
		g.groups = make([]*traceql.Spanset, 0)
	...
	g.at.Reset()
	g.at.AppendOtherValue(otherEntrySpansetKey, ss)
	return &g.at, nil
}

func (g *traceGroupIterator) SeekTo(t parquetquery.RowNumber, definitionLevel int) (*parquetquery.IteratorResult, error) {
	return g.inner.SeekTo(t, definitionLevel)
}

func (g *traceGroupIterator) Close() {
	g.inner.Close()
}
